"""Platform-specific exception hierarchy for wg-automate.
...
class PrivilegeError(PlatformError):
    """Raised when wg-automate is not running as root/Administrator.
    ...
    def __init__(self, message: str | None = None) -> None:
        if message is None:
            if sys.platform == "win32":
                message = (
                    "wg-automate requires root/Administrator privileges. "
                    "Re-run from an elevated command prompt (Run as Administrator)"
                )
            else:
                message = (
                    "wg-automate requires root/Administrator privileges. "
                    "Re-run with: sudo wg-automate"
                )
        super().__init__(message)


class UnsupportedPlatformError(PlatformError):
    """Raised when the current OS is not supported by wg-automate."""
