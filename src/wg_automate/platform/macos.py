"""macOS platform adapter for wg-automate.
...
class MacOSAdapter(AbstractPlatformAdapter):
    """macOS-specific platform adapter for wg-automate.
    ...
    _PF_TOKEN_PATH = Path("/var/run/wg-automate/pf_token")
    ...
    def check_privileges(self) -> None:
        ...
        if sudo_user:
            print(
                ...
                f"The vault will be created under /Users/{sudo_user}/.wg-automate/ "
    ...
    def apply_firewall_rules(
        ...
        rules = (
            f"# wg-automate managed rules -- DO NOT EDIT\n"
    ...
    def enable_ip_forwarding(self) -> None:
        ...
        plist_path = Path("/Library/LaunchDaemons/com.wg-automate.sysctl.plist")
    ...
    def enable_tunnel_service(self, interface: str = "wg0") -> None:
        ...
        plist_bytes = plistlib.dumps(plist_data)
        plist_path = Path(
            f"/Library/LaunchDaemons/com.wg-automate.{interface}.plist"
        )
        ...
        label = f"system/com.wg-automate.{interface}"
    ...
    def setup_dns_updater(
        ...
        Creates the ``wg-automate`` system user if it does not exist, then
        ...
        plist_bytes = plistlib.dumps(plist_data)
        plist_path = Path("/Library/LaunchDaemons/com.wg-automate.dns.plist")
    ...
    def _ensure_wg_automate_user(self) -> None:
        """Create the ``wg-automate`` system user if it does not exist.
        ...
        dscl_cmds = [
            ...
            ["dscl", ".", "-create", "/Users/wg-automate", "UniqueID", str(uid)],
            ["dscl", ".", "-create", "/Users/wg-automate", "PrimaryGroupID", str(gid)],
            [
                "dscl", ".", "-create", "/Users/wg-automate",
    ...
    def _find_available_uid(self, low: int, high: int) -> int:
        ...
        raise SetupError(
            f"No available UID in range {low}-{high} for wg-automate system user"
        )
