"""Linux platform adapter for wg-automate.
...
Security properties:
  ...
  - DuckDNS updater runs as non-root wg-automate system user (HARD-04)
...
_CRON_FILE = Path("/etc/cron.d/wg-automate")
...
class LinuxAdapter(AbstractPlatformAdapter):
    ...
    def check_privileges(self) -> None:
        ...
        if sudo_user:
            print(
                ...
                f"directory (~root/.wg-automate/), not /home/{sudo_user}/.wg-automate/",
                file=sys.stderr,
            )
    ...
    def remove_firewall_rules(self, wg_interface: str) -> None:
        """Remove all wg-automate nftables tables and the drop-in rule file.
    ...
    def setup_dns_updater(
        ...
        """Create the wg-automate system user and schedule DuckDNS updates via cron.

        HARD-04: DuckDNS runs as non-root wg-automate system user, not as root.

        Steps:
          1. Create system user wg-automate if not already present.
          2. Write /etc/cron.d/wg-automate with correct ownership and permissions.

        Args:
            script_path:      Path to the wg-automate CLI entry point.
        ...
        if user_check.returncode != 0:
            try:
                ...
                stderr = exc.stderr.decode("utf-8", errors="replace") if exc.stderr else ""
                raise SetupError(
                    f"Failed to create wg-automate system user: {stderr}"
                ) from exc

        # Step 2: Write cron.d entry
        cron_content = (
            "# Managed by wg-automate -- DO NOT EDIT\n"
            f"*/{interval_minutes} * * * * wg-automate {script_path} update-dns --non-interactive\n"
        )
