...
- [x] **PLAT-04**: macOS setup installs WireGuard via Homebrew, writes config with 600 permissions, enables IP forwarding via `/etc/sysctl.conf`, applies pfctl deny-by-default firewall anchor, configures launchd plist for auto-start and DuckDNS updates
